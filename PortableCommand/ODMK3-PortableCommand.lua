...
local cabinLowered = false -- Track cabin pulley state
        ...
        if file then
            ...
            if data and data ~= "" then
                ...
                if success and state then
                    ...
                    cabinLowered = state.cabinLowered or false
...
local function drawPageHeader(w, h)
  ...
  local pageText = string.format("Page %d/%d - %s", currentPage, totalPages, 
    currentPage == 1 and "Movement" or "Controls")
...
local function layoutUtilitiesPage(w, h, top, bottomStatus)
  ...
  -- List-style layout for controls
  local itemHeight = 3
  local gap = 1
  local totalItems = 4
...
local function press(id, label)
  ...
  elseif id == "CABIN_PULLEY" or id == "CABIN_PULLEY_STATUS" then
    if NET_OK then
      local newState = not cabinLowered
      cabinLowered = newState
      saveState()  -- Save state when making changes
      ...
      -- Update button appearance for both main button and status box
      for _, b in ipairs(buttons) do
        if b.id == "CABIN_PULLEY" then
          -- Keep main button gray but update text if needed
        elseif b.id == "CABIN_PULLEY_STATUS" then
          b.bg = newState and colors.lime or colors.red
          b.label = newState and "ON" or "OFF"
        end
      end
      lastStatusMsg = "Cabin " .. (newState and "LOWERING" or "RAISING")
    else
      lastStatusMsg = "Cannot control cabin: offline"
    end
    redraw()
...
local function handleCollectionStatus(msg)
  ...
  elseif msg.type == "cabinPulleyStatus" then
    if cabinLowered ~= msg.lowered then
      cabinLowered = msg.lowered
      stateChanged = true
    end
    -- Update status box button
    for _, b in ipairs(buttons) do
      if b.id == "CABIN_PULLEY_STATUS" then
        b.bg = msg.lowered and colors.lime or colors.red
        b.label = msg.lowered and "ON" or "OFF"
        drawButton(b)
        break
      end
    end
    lastStatusMsg = "Cabin: " .. (msg.lowered and "LOWERED" or "RAISED")
  end
  
  -- Save state whenever we receive a status update
  if stateChanged then
    saveState()
    print("[STATE] Updated state from controller")
  end
