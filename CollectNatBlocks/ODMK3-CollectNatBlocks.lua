-- ODMK3-CollectNatBlocks.lua
-- Omni-Drill MKIII: Natural Blocks Collection Controller
-- Controls redstone output to Create Item Funnel for natural block collection

-- ========== Configuration ==========
local PROTOCOL = "Omni-DrillMKIII"
local MY_NAME = "odmk3-collect-nat-blocks"
...
-- ========== Persistence ==========
local STATUS_TYPE = "natBlocksStatus"
...
local function handleInitialState(msg)
    if type(msg) ~= "table" then return end
    if msg.type ~= "natBlocksStatus" then return end
    if SECRET ~= "" and msg.secret ~= SECRET then return end
...
local function handleSnapshot(msg)
    ...
    if collectionEnabled == nil then
        if msg.state.collectNatBlocksEnabled ~= nil then
            collectionEnabled = msg.state.collectNatBlocksEnabled and true or false
            updateRedstoneOutput()
            savePersistedState(collectionEnabled)
...
-- ========== main function ==========
local function main()
    print("ODMK3 Natural Blocks Collection Controller")
    print("==========================================")
    ...
    print("ODMK3-CollectNatBlocks initialized")
    ...
    while true do
        ...
        elseif event == "terminate" then
            print("Natural Blocks Collection Controller shutting down")
            break
        end
    end
end
