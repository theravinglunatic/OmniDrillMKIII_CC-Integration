-- ODMK3-GantryShift.lua
-- Monitors machine orientation (cardinal + vertical) and emits redstone signals
-- to reverse gantry direction when needed based on current orientation.
...
local function updateRedstoneOutput()
    ...
    elseif currentCardinal == "W" and currentVertical == "F" then
        shouldEmitSignal = false
    -- When the machine is facing North-Down, Gantry Shift should be ON
    elseif currentCardinal == "N" and currentVertical == "D" then
        shouldEmitSignal = true
    -- When the machine is facing East-Up, Gantry Shift should be ON
    elseif currentCardinal == "E" and currentVertical == "U" then
        shouldEmitSignal = true
    -- When the machine is facing South-Up, Gantry Shift should be ON
    elseif currentCardinal == "S" and currentVertical == "U" then
        shouldEmitSignal = true
        debugPrint("South-Up orientation detected - Gantry Shift ON")
    -- When the machine is facing West-Down, Gantry Shift should be ON
    elseif currentCardinal == "W" and currentVertical == "D" then
        shouldEmitSignal = true
        debugPrint("West-Down orientation detected - Gantry Shift ON")
    ...
    if shouldEmitSignal ~= lastSignalState then
        redstone.setOutput(REDSTONE_OUTPUT_SIDE, shouldEmitSignal)
        debugPrint("Set redstone output on " .. REDSTONE_OUTPUT_SIDE .. " to " .. 
...
local function main()
    ...
    print("ODMK3-GantryShift initialized")
    debugPrint("Waiting for orientation data")
    
    -- Ensure no redstone output at startup
    redstone.setOutput(REDSTONE_OUTPUT_SIDE, false)
    lastSignalState = false
    debugPrint("Initialized redstone output to OFF")
